...
Responses are shaped to stay compact for MCP clients: document `content` in list/search results is truncated to a snippet (`full_content: true` opts out; `document_get` returns full text), the unbounded `all` ID array is stripped from paginated lists (`include_all_ids: true` on `document_list` re-includes it), `task_list` is paginated client-side, `log_get` returns only the last `tail` lines (default 100), `document_selection_data` omits zero-count objects, `user_list` omits `inherited_permissions` unless `include_permissions: true`, and `profile_get` redacts the API auth token.
