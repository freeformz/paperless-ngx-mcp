...
## Response Shaping
Responses are trimmed to stay compact. Document content in document_list/search_global results is truncated to a snippet (default 500 bytes, content_truncated: true marks truncated entries) — adjust with content_snippet_bytes (0 disables), pass full_content: true, or use document_get for full text. Paginated lists omit the "all" ID array (include_all_ids: true on document_list re-includes it for bulk workflows). log_get returns only the last 100 lines by default (tail parameter). document_selection_data omits objects with zero matching documents. user_list omits inherited_permissions unless include_permissions: true.
